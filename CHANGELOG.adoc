...
* If your site references (e.g., in software, spec frontmatter)
  repositories using predominantly `master` as main branch name,
  set `default_repo_branch` in `_config.yml` to "master".
* If your site references repositories that
  use another main branch name
  than the one you specify in `default_repo_branch`
  (or, absent that setting, other than `main`),
  find those references and add sibling
