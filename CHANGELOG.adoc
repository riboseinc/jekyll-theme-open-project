...
which will be used as default for software/spec within the site.
...
```
in `command': git '--git-dir=/site/path/some/repository/path/.git' '--work-tree=/site/path/some/repository/path/' '-c' 'color.ui=false' checkout '--force' 'origin/some_branch_name'  2>&1:error: pathspec 'origin/some_branch_name' did not match any file(s) known to git (Git::GitExecuteError)
```
...
* If your site contents (e.g., software, spec frontmatter) references
  ...
  properties in the form `*_repo_branch`. E.g., if your 
